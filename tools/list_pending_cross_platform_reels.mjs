...
const pending = history.records
  ...
  .map((record, index) => ({
    review_id: `P${String(index + 1).padStart(2, '0')}`,
    ...
    drive_asset_evidence: record.drive_asset_evidence
      ...
      : null,
    review_status: 'Pendiente_de_confirmacion_de_Fernando',
  }))
...
const artifact = {
  ...
  pieces: pending,
  decision_rule: 'Solo se agregará una relación cross-platform cuando Fernando la confirme o exista una coincidencia verificable de IDs, copy, asset y ventana temporal.',
}

const target = path.join(root, 'Operations/Research/2026-08-19_Piezas_Sin_Cascada_Revision.json')
