...
Esta implementación está en `Review`. No autoriza compilar, importar, ejecutar, probar, registrar nombres reales ni observar el sistema. Ante la ausencia local de `pytest`, se creó y revisó el runner estándar `run_policy_core_gsec211_synthetic.py`, documentado en `2026-08-27_Runner_Sintetico_Estandar_GSEC2_11_USM.md` v1.2 en `Review`. El runner no se ha importado ni ejecutado. Cualquier prueba sintética ejecutada necesitará una autorización humana nueva y no autorizará observación del sistema, nombres reales ni una integración.
