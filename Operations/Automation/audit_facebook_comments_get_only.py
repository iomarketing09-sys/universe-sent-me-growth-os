"""Read-only Facebook comment scan using the latest successful review as cursor.

The scan never writes to Meta. It derives the Page token in memory, reviews the
20 most recent own Page posts, reads roots and one level of replies, and emits a
new timestamped artifact without overwriting prior evidence.
"""
...
review_candidates = list(RESEARCH.glob("*_Facebook_Comment_Review_GET_Only.json"))
if not review_candidates:
    raise SystemExit("NO_PREVIOUS_GET_ONLY_REVIEW_FOR_CURSOR")
latest_review_path = max(
    review_candidates,
    key=lambda path: json.loads(path.read_text(encoding="utf-8")).get("reviewed_at", ""),
)
latest_review = json.loads(latest_review_path.read_text(encoding="utf-8"))
cursor = parse_dt(latest_review["reviewed_at"])
reviewed_at = datetime.now(timezone.utc)
OUT = RESEARCH / f"{reviewed_at.strftime('%Y-%m-%d_%H-%M-%S')}_Facebook_Comment_Review_GET_Only.json"
...
for post, root in roots:
    ...
    for comment, comment_type, parent_id, page_reply_count in units:
        ...
        row = {
            ...
            "created_after_latest_cursor": created >= cursor,
        }
        if created >= cursor:
...
result = {
    "title": "Facebook Comment Review — GET-only timestamped cut",
    "purpose": "Revisión de solo lectura de comentarios nuevos y oportunidades potenciales de engagement desde el último corte exitoso, incluyendo raíces y réplicas de las 20 publicaciones propias más recientes.",
    "status": "Review",
    ...
    "author": "Manus AI",
    "related_documents": [
        str(latest_review_path.relative_to(ROOT)),
        "Operations/Research/2026-08-15_Community_Engagement_Log.csv",
        "Operations/Research/2026-08-15_Auditoria_Comentarios_Facebook.md",
    ],
    "organization": "Operations/Research",
    "reviewed_at": reviewed_at.isoformat(timespec="seconds"),
    "cursor_source": str(latest_review_path.relative_to(ROOT)),
    "cursor": latest_review["reviewed_at"],
    "page_id": PAGE_ID,
    "source": "Meta Graph API v26.0 / Page feed + direct post comments + one-level nested replies; no pagination beyond the first 100 per collection",
    "read_only": True,
    ...
    "new_units_since_latest_cursor": len(new_since_cursor),
    "new_unanswered_not_in_ledger_since_latest_cursor": len(new_unanswered_since_cursor),
    "logged_unanswered_since_latest_cursor": len(logged_unanswered_since_cursor),
    "page_replied_units_since_latest_cursor": len([row for row in page_replied if row["created_after_latest_cursor"]]),
    "api_error_count": len(api_errors),
    "api_errors": api_errors,
    "new_unanswered_not_in_ledger": new_unanswered_since_cursor,
    "logged_unanswered_since_latest_cursor": logged_unanswered_since_cursor,
    "all_current_unanswered": unanswered,
}
OUT.write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
summary = {key: result[key] for key in ("reviewed_at", "cursor", "posts_fetched", "page_posts_reviewed", "root_comments_seen", "comment_ids_seen", "current_unanswered_units", "new_units_since_latest_cursor", "new_unanswered_not_in_ledger_since_latest_cursor", "logged_unanswered_since_latest_cursor", "page_replied_units_since_latest_cursor", "api_error_count")}
summary["artifact"] = str(OUT.relative_to(ROOT))
