"""Read-only Facebook comment scan for Batch 14.
...
new/unanswered candidates after the verified Batch 13 cursor.
"""
...
BATCH13 = ROOT / "Operations/Research/2026-08-24_Facebook_Comment_Publication_Batch_13.json"
OUT = ROOT / "Operations/Research/2026-08-24_Facebook_Comment_Review_Batch_14.json"
POST_LIMIT = 20
MAX_WORKERS = 8
TIMEOUT = 30
...
batch13 = json.loads(BATCH13.read_text(encoding="utf-8"))
cursor = parse_dt(batch13["published_at"])
...
for post, root in roots:
    ...
    for comment, comment_type, parent_id, page_reply_count in units:
        ...
        row = {
            ...
            "created_after_batch13_cursor": created >= cursor,
        }
        if created >= cursor:
...
result = {
    "title": "Facebook Comment Review Batch 14",
    "purpose": "Revisión de solo lectura de comentarios nuevos y oportunidades potenciales de engagement después del Batch 13, incluyendo raíces y réplicas de las 20 publicaciones propias más recientes.",
    "status": "Review",
    ...
    "cursor_source": "2026-08-24_Facebook_Comment_Publication_Batch_13.json",
    "cursor": batch13["published_at"],
    "page_id": PAGE_ID,
    ...
    "new_units_since_batch13_cursor": len(new_since_cursor),
    "new_unanswered_not_in_ledger_since_batch13_cursor": len(new_unanswered_since_cursor),
    "logged_unanswered_since_batch13_cursor": len(logged_unanswered_since_cursor),
    "page_replied_units_since_batch13_cursor": len([row for row in page_replied if row["created_after_batch13_cursor"]]),
    "api_error_count": len(api_errors),
    "api_errors": api_errors,
    "new_unanswered_not_in_ledger": new_unanswered_since_cursor,
    "logged_unanswered_since_batch13_cursor": logged_unanswered_since_cursor,
    "all_current_unanswered": unanswered,
}
OUT.write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
print(json.dumps({key: result[key] for key in ("reviewed_at", "cursor", "posts_fetched", "page_posts_reviewed", "root_comments_seen", "comment_ids_seen", "current_unanswered_units", "new_units_since_batch13_cursor", "new_unanswered_not_in_ledger_since_batch13_cursor", "logged_unanswered_since_batch13_cursor", "page_replied_units_since_batch13_cursor", "api_error_count")}, ensure_ascii=False))
