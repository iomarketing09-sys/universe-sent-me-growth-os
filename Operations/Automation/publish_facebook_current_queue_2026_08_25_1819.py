#!/usr/bin/env python3
"""Preflight and publish exactly the five current Facebook queue replies.

The script is deliberately two-phase:
  python ... preflight  # GET-only; writes a preflight evidence artifact
  python ... publish    # requires a passing preflight; POSTs only the five targets
"""
...
PREflight = RESEARCH / "2026-08-25_18-19-20_Facebook_Publication_Preflight.json"
...
TIMEOUT = 30
MAX_PAGES = 20
APPROVAL_AT = "2026-08-25T18:19:20+00:00"
    ...
    try:
        for row in rows:
            ...
            checks.append({
                ...
                "comment_type": row.get("comment_type", "Comentario_Raiz"),
        ...
        blocked = {
            "title": "Facebook Publication Preflight — current queue blocked",
            "purpose": "Evidencia del preflight GET-only de las cinco respuestas actuales; ningún POST fue ejecutado si el preflight no completó.",
            ...
            "related_documents": [
                ...
                "Operations/Research/2026-08-25_18-19-20_Facebook_Approval_and_Filtered_Review.json",
                "Operations/Research/2026-08-15_Community_Engagement_Log.csv",
            ],
            "source": "Meta Graph API v26.0",
            "page_id": PAGE_ID,
            "explicit_user_approval": True,
            "approval_scope": "Las cinco respuestas actuales de la cola aprobadas por Fernando en conversación.",
        ...
        raise
    payload = {
        "title": "Facebook Publication Preflight — current queue",
        "purpose": "Preflight GET-only completo de las cinco respuestas actuales antes de cualquier publicación.",
...
def publish() -> None:
    ...
    try:
        for check, row in zip(pre["checks"], rows):
            ...
            is_nested = row.get("comment_type") == "Replica_Anidada" or bool(target_parent)
            if returned_parent == cid:
                parent_semantics = "direct_target_parent"
                parent_ok = True
            elif is_nested and target_parent and returned_parent == target_parent:
        ...
        partial = {
            "title": "Facebook Publication — current queue partial evidence",
            "purpose": "Evidencia parcial de la publicación de las cinco respuestas actuales; no se reintenta automáticamente.",
    ...
    payload = {
        "title": "Facebook Publication — current queue",
        "purpose": "Evidencia de publicación y verificación de exactamente las cinco respuestas autorizadas de la cola actual.",
        "status": "Active",
        "created_at": started,
        "updated_at": finished,
        "version": "1.0",
...
if __name__ == "__main__":
    if len(sys.argv) != 2 or sys.argv[1] not in {"preflight", "publish"}:
        raise SystemExit("usage: publish_facebook_current_queue_2026_08_25_1819.py preflight|publish")
