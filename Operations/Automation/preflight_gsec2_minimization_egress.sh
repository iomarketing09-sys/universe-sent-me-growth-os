#!/usr/bin/env bash
# G-SEC-2.1a synthetic minimization/egress preflight.
# Design reminder: fixture-only policy verification; never read data or send it anywhere.
set -euo pipefail

readonly CONFIRMATION='RUN_USM_GSEC2_MINIMIZATION_EGRESS_SYNTHETIC'
...
usage() {
  cat <<'EOF'
Usage:
  preflight_gsec2_minimization_egress.sh --plan
  preflight_gsec2_minimization_egress.sh --preflight
  preflight_gsec2_minimization_egress.sh --execute --confirm RUN_USM_GSEC2_MINIMIZATION_EGRESS_SYNTHETIC

Safety limits: fixture-only; no private paths, environment, collector, socket,
OAuth/API, service, scheduler, installation, ledger/evidence, or external output.
EOF
}
...
printf 'USM G-SEC-2.1a synthetic minimization and egress preflight\n'
...
readonly VALIDATOR="$REPO_ROOT/Operations/Automation/validate_gsec2_minimization_egress_synthetic.py"
readonly FIXTURE="$REPO_ROOT/Operations/Automation/fixtures/gsec2_minimization_egress_synthetic.json"
...
if [ "$MODE" = 'preflight' ]; then
  ...
  echo 'STATUS=preflight_complete_gsec2_minimization_egress_synthetic_only_no_network_no_private_read'
  exit 0
fi
[ "$MODE" = 'execute' ] || die 'invalid mode'
[ "$CONFIRM" = "$CONFIRMATION" ] || die 'exact --confirm string is required'
cd "$REPO_ROOT/Operations/Automation"
PYTHONDONTWRITEBYTECODE=1 python3 -B validate_gsec2_minimization_egress_synthetic.py
echo 'STATUS=gsec2_minimization_egress_synthetic_complete_no_network_no_private_read'
