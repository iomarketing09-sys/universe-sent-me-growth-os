...
Posteriormente se conectó Google AI Studio como segundo provider. `gemini/gemini-2.5-flash` respondió HTTP `404` porque ya no estaba disponible para usuarios nuevos; el model ID fue descartado. `gemini/gemini-3.5-flash`, visible en el catálogo, respondió correctamente con HTTP `200`, `finish_reason: stop`, `5359 ms` y `strategy=single; provider=gemini`. La prueba directa valida el segundo provider, pero no demuestra todavía el failover del Combo. El Combo `usm-groq-gemini-priority` fue probado hasta ahora con Groq como ruta primaria; no debe considerarse failover validado hasta observar una solicitud atendida por Gemini después de un fallo controlado de Groq. La diferencia entre los tokens `usage` del cuerpo (`39/623`) y los headers de OmniRoute (`39/34`) queda registrada como observación de medición.
