...
El Combo queda aprobado para staging y evaluación editorial manual. No queda aprobado para ser llamado desde el navegador ni para producción mientras OmniRoute permanezca en el iMac. La integración de producción debe utilizar un backend privado con HTTPS, autenticación del cliente, payload mínimo, límites, rate limiting, timeouts y logs; ese backend puede invocar el Combo por su nombre exacto y devolver únicamente una respuesta marcada como derivada/Draft. El siguiente entregable técnico es definir `POST /api/ai/draft`, sin escritura automática en los ledgers ni publicación social.
