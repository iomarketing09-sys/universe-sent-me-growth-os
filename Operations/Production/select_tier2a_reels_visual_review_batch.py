...
unique={}
for row in queue: unique.setdefault(row['Platform_Content_ID'],row)
selected=[row for row in unique.values() if 16<=int(row['Priority_Rank'])<=25]
folders={f.get('id'):f for f in tree['files'] if f.get('mimeType')=='application/vnd.google-apps.folder'}
def path_for(parents,guard=0):
    if guard>8 or not parents:return ''
    f=folders.get(parents[0]); return '' if not f else f"{path_for(f.get('parents') or [],guard+1)}/{f.get('name')}".strip('/')
def dt(s):
...
def toks(s):return {w for w in re.findall(r'[a-záéíóúüñ0-9]+',(s or '').lower()) if len(w)>=4 and w not in generic}
assets=[]
for a in tree['files']:
    if a.get('mimeType','').startswith(('video/','image/')):
        a['folder_path']=path_for(a.get('parents') or []);assets.append(a)
rows=[]
for p in selected:
    pub=dt(p['Publication_UTC']); pt=toks(p['Title_or_Caption']); scored=[]
    for a in assets:
        dates=[dt(a.get('createdTime','')),dt(a.get('modifiedTime',''))]; dates=[d for d in dates if d]
    ...
    for rank,(score,delta,overlap,a) in enumerate(scored[:5],1):
        rows.append({'Review_Batch':'TIER2A','Meta_Priority_Rank':p['Priority_Rank'],'Meta_Post_ID':p['Platform_Content_ID'],'Meta_Reel_ID':p['Meta_Reel_ID'],'Publication_UTC':p['Publication_UTC'],'Engagement':p['Engagement'],'Meta_Caption':p['Title_or_Caption'],'Candidate_Rank':rank,'Drive_File_ID':a.get('id'),'Drive_File_Name':a.get('name'),'Drive_Folder_Path':a.get('folder_path'),'Drive_MimeType':a.get('mimeType'),'Drive_Created_UTC':a.get('createdTime'),'Drive_Duration_ms':(a.get('videoMediaMetadata') or {}).get('durationMillis'),'Date_Delta_Days':round(delta,2),'Meaningful_Token_Overlap':'|'.join(overlap),'Triage_Score':round(score,2),'Review_Status':'Needs_visual_review'})
out=ROOT/'Operations/Research/2026-08-22_Reels_Tier2A_Visual_Review_Batch.csv'
