# CNT-029 — Reel "Pausa para ver qué piensa de ti" (6 cuadros de sueños con Universe)

| Campo | Valor |
| :--- | :--- |
| **Propósito** | Registro del reel de 6 imágenes con composición idéntica (Universe sentado en una nube con globo de pensamiento sobre el cielo pastel) en la que solo cambia el sueño dentro del globo; la frase del reel "Pausa para ver qué piensa de ti" va como texto superpuesto en el video, no en las imágenes |
| **Estado** | Draft (pendiente de aprobación de Fernando) |
| **Fecha de creación** | 2026-08-14 |
| **Última actualización** | 2026-08-14 (v2.1: sección de portadas y hooks añadida) |
| **Versión** | 2.1 |
| **Autor** | Manus AI (CGO) |
| **ID** | CNT-029 |
| **Documentos relacionados** | `GrowthOS/03_00_Sistema_Generacion_Memes.md`, `GrowthOS/12_00_Sistema_Dos_Capas_Contenido_Canon.md`, `Operations/Production/CNT028_Memes_Adaptados_Drive_Frase_Intacta.md` |

## 1. Concepto del reel

La imagen base fue creada por Fernando (13-ago-2026) y sirve de plantilla: Universe sentado en una nube sobre un cielo pastel de ensueño (islas flotantes, castillo rosa, globos aerostáticos, rayos dorados) con un globo de pensamiento donde aparece su sueño. El formato del reel es de cuadros idénticos donde **solo cambia el sueño**, con el texto "Pausa para ver qué piensa de ti" como superposición de video (no incrustado en las imágenes). El contenido alterna humor y romance, en línea con el territorio "love" validado del Growth OS (los posts de ternura/romance con Universe tienen el ER más alto del periodo, 3.31% en el post "señales del universo").

## 2. Los 6 cuadros (tu imagen + 5 generadas)

| # | Archivo (asset del proyecto) | El sueño dentro del globo | Tono |
| :--- | :--- | :--- | :--- |
| F1 | Imagen base de Fernando (sábana negra a rayas, Universe acostado en la falda de ella) | Universe acostado en la falda de la chica, feliz | Romance |
| F2 | `06_Reel_Piernas_universo_sueno_20260814.png` | Universe recostado de espaldas en las piernas de ella mientras le acaricia la panza | Romance |
| F3 | `07_Reel_Pecho_universo_sueno_20260814.png` | Universe acurrucado durmiendo sobre el pecho de ella, abrazado | Romance |
| F4 | `08_Reel_Cena_romantica_universo_sueno_20260814.png` | Universe siendo alimentado con pastel en una cena romántica con velas | Romance / humor |
| F5 | `09_Reel_Lluvia_cacao_universo_sueno_20260814.png` | Universe y ella bajo una cobija con cacao mientras llueve afuera | Romance / humor |
| F6 | `10_Reel_Atardecer_sueno_20260814.png` | Universe con mini-lentes oscuros viendo el atardecer en la colina con ella | Humor (cierre cómico) |
| F7 | `11_Reel_Pecho_picaresca_universo_sueno_20260814.png` | Universe en el pecho de ella sobre la cama con pétalos y velas, mirada cómplice (variante pícaro-sensual) | Romance pícaro |
| F8 | `12_Reel_Atado_ribbon_universo_sueno_20260814.png` | Universe "cautivo" con lazo de seda rojo juguetón en las patas, cara de felicidad total (comedia BDSM PG) | Humor pícaro |
| F9 | `13_Reel_Coqueto_rosa_universo_sueno_20260814.png` | Universe con moño de lazo entregándole una rosa en la cena, ceja levantada estilo Casanova | Humor pícaro |

**Nota de tono (v2.0):** a solicitud de Fernando se añadieron 3 cuadros "pícaros" (F7–F9) como extensión del reel original de 6 cuadros, quedando el banco con 9 cuadros en total. Se mantuvieron deliberadamente en clave de comedia romántica con estética tierna: nada explícito, sin desnudez ni contenido sexual — la línea de USM es "ácido y pícaro, nunca explícito". Este equilibrio permite el doble sentido que genera comentarios y shares, sin riesgo de restricción de alcance en Meta/TikTok ni de la línea editorial del proyecto. Recomendar usar los cuadros pícaros en posiciones intermedias del reel (nunca como thumbnail ni como primer cuadro), para que el video entre con la versión tierna y el giro pícaro sea la sorpresa.

Todas las imágenes respetan la identidad física canónica de Universe (gato blanco con gafas steampunk doradas de engranajes, asset oficial 3) y el mismo estilo de animación 2D cartoon pastel de la imagen base. Cada imagen lleva la marca "UniverseSentMe" discreta.

## 2b. Portadas y hooks de texto (v2.1)

Cuatro conceptos de portada (P1 close-up de Universe con cara de secreto; P2 globo de pensamiento borroso con "⬇ PAUSA PARA LEER ⬇"; P3 split "lo que piensas tú vs. lo que piensa él"; P4 portada "glitch" con el sueño censurado) y cinco hooks (H1 directo de pausa; H2 POV secreto; H3 reto "90% no pausará"; H4 pregunta abierta con promesa; H5 "los pensamientos se ponen intensos a partir del frame 5 👀"). Recomendados: P2 o P4 de portada y H5 o H3 de hook. Copy sugerido con CTA de etiquetar y hashtag de formato emergente `#PausaParaVerQuePiensa`. Detalle completo en `/home/ubuntu/portada_hook_reel_suenos.md` (entregable) y métricas a registrar: watch-time, retención al segundo 5 (>60% umbral), replays y comentarios que mencionen la pausa.

## 3. Guion sugerido del reel (v2.0 — corregido por Fernando)

El formato final es **scroll ultra rápido con gancho de pausa**: cada imagen dura ~0.5–1 s (el texto de cada sueño apenas es legible al vuelo), lo que obliga al espectador a pausar el video para leer los pensamientos — mecánica de watch-time que premia el algoritmo. Montaje: texto superpuesto "Pausa para ver qué piensa de ti ✨" solo en el primer instante, imágenes en secuencia rápida de tiernas a pícaras/irónicas, cierre con el cuadro de remate cómico, audio trending estilo Måneskin (rock con actitud) que contrasta con la ternura visual. Duración total objetivo: ~6–9 s en loop. Detalle técnico y referencias de audio en `CNT030_Musica_Reel_Suenos_Universe.md` (v2.0).

## 3b. Guion original sugerido (v1.0 — reemplazado)

Duración por cuadro: 1.5–2 segundos, 6 cuadros ≈ 10–12 segundos (formato corto con alta probabilidad de re-watch). Audio sugerido: canción de piano/lo-fi tierna o un trending de romance. Texto superpuesto en el primer cuadro: **"Pausa para ver qué piensa de ti"**; en los siguientes cuadros puede añadirse un emoji sutil (🧎 💤 🎂 ☔ 🕶️) pero nunca otra frase, para que el texto único del reel sea el gancho.

**Copy sugerido:** "Pausa para ver qué piensa de ti ✨ (Universe siempre la piensa a ella) #UniverseUSM #UniverseSentMe #ReelsUSM". **Slot sugerido:** domingo 17 de agosto, 7:00 PM (domingo es el día de mayor ER% del proyecto, 1.77%).

## 4. Canon y sistema de dos capas

La figura femenina es genérica y repetida (sin rasgos definidos que la identifiquen como personaje nombrado), por lo que el reel no fija ningún vínculo nuevo de canon: se mantiene en Capa 1. Si en el futuro se decidiera que esta chica es un personaje recurrente (Maeve, por ejemplo), requeriría validación explícita de Fernando y una ficha de diseño, dado que Maeve aún no tiene asset oficial en el proyecto (regla dura nº 5 del `03_00`).
