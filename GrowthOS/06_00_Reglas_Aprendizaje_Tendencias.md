# Reglas Estratégicas del CGO: Aprendizaje, Tendencias y Temporalidad (v3.0)
...
## 1. Cadencia por Plataforma

Para maximizar el crecimiento y la retención, se establecen los siguientes horarios y frecuencias:

### Facebook (Motor de Crecimiento)
- **Frecuencia:** Mínimo 4 posts diarios.
- **Horarios:** 10:30 AM, 4:00 PM, 7:00 PM, 9:00 PM.
- **Formato:** Fotos (Memes), Comics, Reels.

### Instagram / TikTok / YouTube Shorts (Motor de Adquisición)
- **Frecuencia:** 1-2 Reels diarios.
- **Regla de Cascada:** Todo Reel publicado en Instagram debe replicarse en TikTok y YouTube Shorts para maximizar el alcance sin esfuerzo adicional.

---

## 2. Reglas de Reutilización y Temporalidad

### Criterios de Reutilización
Nunca reutilices contenido únicamente porque existe. Antes de recomendar una reutilización, el CGO debe validar:
- **Antigüedad:** Mínimo 30 días desde la última publicación.
- **Contexto:** ¿Es relevante para el clima social o los objetivos actuales?
- **Rendimiento Histórico:** Priorizar piezas con alto engagement (>5% en Facebook).
- **Saturación:** Evitar repetir el mismo personaje o formato en días consecutivos.

### Clasificación Temporal
- **Evergreen:** Contenido atemporal (Relatable, Absurdo, Filosofía). Puede usarse en cualquier momento.
- **Estacional:** Vinculado a fechas específicas.
  - *Día de las Madres* → Solo en Mayo.
  - *Halloween* → Solo en Octubre.
  - *Navidad* → Solo en Diciembre.
- **Evento/Tendencia:** Vinculado a una noticia o meme del momento. Requiere validación inmediata.

**Prohibición Estricta:** Nunca propongas contenido cuya temporalidad no corresponda con la fecha actual.

---

## 3. Aprendizaje Post-Publicación

Después de cada publicación deberás actualizar automáticamente el conocimiento del sistema. No te limites a almacenar datos. Extrae conclusiones, actualiza hipótesis y detecta patrones.

### Métricas a Analizar
- Alcance, Reproducciones, Compartidos, Guardados, Comentarios, CTR, Retención y Seguidores obtenidos.

### Acciones Post-Publicación
1. **Extraer conclusiones:** ¿Por qué funcionó o falló esta pieza?
2. **Actualizar hipótesis:** Si la pieza validó una `Hipotesis_ID`, actualizar su estado en el Backlog.
3. **Detectar patrones:** Identificar qué formatos, personajes u horarios están generando mayor engagement.
...
1. **Alineación Canon:** ¿Encaja con la narrativa de los personajes?
2. **Alineación Tono:** ¿Respeta el humor existencial / filosófico / absurdo del universo?
3. **Riesgo de Marca:** ¿Puede dañar la percepción del proyecto?

---

## 5. Filosofía de Decisión

Ante varias opciones, selecciona siempre aquella que:
1. Aporte mayor aprendizaje (validación de nuevas hipótesis).
2. Tenga mayor probabilidad de crecimiento basada en datos históricos.
3. Fortalezca el universo narrativo (Canon Guarding).
4. Optimice el tiempo de producción (IA-first).

---

## 6. Notas de Copy y Hashtags por Plataforma (2026-08-02)
...
Se recuperó el texto exacto de Meta para 16 de los 17 casos aprobados de personajes; un caso usa fallback de la cola porque Meta no devolvió mensaje. La revisión manual de los 17 casos queda cerrada y deja el corte en 8 `caption_minimo`, 2 `caption_conversacional`, 6 `caption_refuerzo` y 1 `historical_unavailable`. Todos tienen `manual_review_status=Analyst_Reviewed`; 12 cuentan con confianza alta y 5 con confianza media.

La revisión confirmó que el caption de Ganso repite el texto visual y pasa a `caption_minimo`; Universe conserva `caption_refuerzo`; la mujer mágica, Universe de reacción, Silvio y Wilfred interrogativo también quedan como `caption_refuerzo`; el roster mixto y Universe con CTA quedan como `caption_conversacional`; los captions de hashtags, emojis o acompañamiento quedan como `caption_minimo`; y el Fantasma sin mensaje queda como `historical_unavailable`. No se debe actualizar el ExperimentLog ni declarar efecto de tratamiento. Para comparar captions dentro de una celda se requieren al menos dos casos por tratamiento, estructura comparable y control de confusores; este subconjunto fue seleccionado por utilidad visual y no cumple ese balance.

### 18.5 Implicaciones estratégicas de captions — 2026-08-21

La auditoría manual de los 17 casos queda cerrada, pero sigue siendo una muestra seleccionada por utilidad visual de personaje y no un experimento balanceado. Ganso mueve 14 interacciones, 2 shares y 3 comentarios de `caption_refuerzo` a `caption_minimo`; Universe permanece en `caption_refuerzo` con 164 interacciones y 42 shares. Ningún post cambia sus métricas y el total del subconjunto permanece en 300 interacciones, 53 shares y 13 comentarios. El outlier de Universe hace que cualquier lectura agregada de `caption_refuerzo` sea sensible a composición.

A partir de esta revisión, `Caption_Treatment` debe separarse de `Caption_Function`. El primero registra `caption_minimo`, `caption_refuerzo` o `caption_conversacional`; el segundo registra la función observada: `hashtag_acompanamiento`, `reaccion`, `refuerzo_semantico`, `cta_agradecimiento` o `pregunta_abierta`. Una pregunta retórica no se clasificará automáticamente como conversación. Un caption breve con voz interpersonal puede ser `caption_refuerzo` aunque no invite a responder.
...
El preflight de `FUT-MICRO-005`, `FUT-MICRO-006`, `FUT-TRANS-003` y `FUT-ACID-003` pasó con `4/4` especificaciones en `PASS`, y la matriz ya contiene propuestas completas para los metadatos obligatorios. La validación cruzada no encontró colisiones directas, pero la simulación mostró que una doble asignación a una familia de `n=3` infla el denominador `33.33%`; por tanto, cada brief debe conservar su `Cell_ID` y no entrar automáticamente en FAM-02/FAM-03/FAM-04/FAM-05. Las hipótesis ya están registradas como HB-006 a HB-009 en el HypothesisBank y Fernando aprobó la generación exclusiva de los cuatro assets el 2026-08-21. Los cuatro assets ya fueron generados y el siguiente gate es completar el control visual, especialmente la revisión de identidad de Universe. La aprobación de generación no autoriza calendario, publicación, CNT, reuse ni afiliados. No se abrirá otra ronda histórica de captions ni se actualizará el ExperimentLog con estas etiquetas descriptivas.
...
## 19. Respuestas comunitarias: especificidad sobre ingenio — 2026-08-22

La revisión humana de un lote real de comentarios de Facebook y sus respuestas refinadas establece una regla operativa para Community Growth: la calidad de una respuesta no depende de que sea la frase más ingeniosa, sino de que demuestre que la Página leyó el comentario concreto. Esta regla complementa la gestión de comentarios documentada en `Operations/Research/2026-08-15_Community_Engagement_Log.md` y la auditoría estratégica de Facebook en `Operations/Research/2026-08-15_Auditoria_Comentarios_Facebook.md`.

| Señal | Tratamiento recomendado | Evitar |
|---|---|---|
| Comentario musical con canción concreta | Referirse a la canción, al recuerdo o al significado que la persona compartió. | Responder con el mismo elogio genérico para todas las canciones. |
| Historia personal de duelo o pérdida | Usar una frase breve, sencilla y empática, sin humor. | Sonar como tarjeta de felicitación, diagnosticar o trivializar. |
| Enlace musical | Agradecerlo o convertirlo en una “playlist imaginaria” claramente humorística. | Afirmar que la Página añadió la canción a una playlist real si no existe. |
| Reacción breve o emoji | Responder con una línea de personalidad cuando aporte cercanía. | Sobreescribir el momento con una explicación larga o una pregunta forzada. |
...
## 25. Automatización de revisión Facebook — prueba aprobada de siete días

Fernando autorizó automatizar la cadencia focalizada. La tarea recurrente `Revisión Facebook USM — cadencia focalizada` se creó el 2026-08-25 y fue pausada preventivamente durante el bloqueo de acceso; después de que Fernando confirmó la activación del conector Meta, se verificó el acceso y se reactivó de forma restringida. Conserva la expiración del 2026-09-01 a las 10:12:56 UTC. La pauta útil es de tres revisiones diarias a las **12:00, 17:30 y 21:30**, hora de `America/Matamoros`, mediante Meta Graph API v26.0 y operaciones GET-only.

Por una limitación del cron de seis campos, la tarea tiene seis disparos técnicos —12:00, 12:30, 17:00, 17:30, 21:00 y 21:30—. Los disparos de 12:30, 17:00 y 21:00 deben terminar en `no_op` sin consultar otras redes ni modificar la cola. Los conectores permitidos están restringidos a `Universe Sent Me Meta API` y `GitHub`; no se heredan navegador, Instagram, Make, Windsor, Canva, Calendar ni Meta Ads.

El comportamiento de cada revisión útil es: leer el delta con cursor incremental, preservar todos los IDs estructurales en artefactos fechados, separar raíces y réplicas, clasificar propuestas específicas y casos `No_Requiere_Respuesta`, preparar un reporte compacto para Fernando y sincronizar la trazabilidad oficial. Si no hay novedades, informa cero y no cambia la cola.

La barrera de escritura es obligatoria: la tarea no puede ejecutar POST, PUT, DELETE, publicación, ocultamiento ni modificación en Meta. Tampoco debe publicar respuestas previamente aprobadas; cada lote requiere una autorización posterior y explícita de Fernando. No se conservan nombres ni IDs personales de autores. **Estado:** Active, prueba de siete días. **Schedule ID:** `4i8525UwBbh8mk84iZZ42Y`. **Conectores permitidos:** `Universe Sent Me Meta API` y `GitHub`, sin herencia de otros conectores. **Evidencia de verificación:** `Operations/Research/2026-08-25_18-10-48_Facebook_Schedule_Reactivation_Verification.json`. **Documentos relacionados:** `GrowthOS/14_00_Fuente_Maestra_y_Ledgers.md`, `Operations/Research/2026-08-15_Community_Engagement_Log.md`, `Operations/Research/2026-08-15_Auditoria_Comentarios_Facebook.md` y `GrowthOS/00_01_Changelog_GrowthOS.md`.
